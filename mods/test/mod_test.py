...
class Test(IModule):
    ...
    class ModConfModel:
        ...
        just use pass | if you leave it empty, in the load() method just
        init empty object ==> self.ModConfig = ModConfModel()
        """
        param_exemple1: str
        param_exemple2: int
    ...
    async def create_tables(self) -> None:
        """Methode qui va créer la base de donnée si elle n'existe pas.
           Une Session unique pour cette classe sera crée, qui sera utilisé
           dans cette classe / module
    ...
    async def load(self) -> None:
        ...
        _commands = self.ctx.Commands
        _commands.build_command(0, self.module_name, 'test-command',
                                'Execute a test command')
        _commands.build_command(0, self.module_name, 'asyncio',
                                'Create a new asynchron task!')
        _commands.build_command(1, self.module_name, 'test_level_1',
                                'Execute a level 1 test command')
        _commands.build_command(2, self.module_name, 'test_level_2',
                                'Execute a level 2 test command')
        _commands.build_command(3, self.module_name, 'test_level_3',
                                'Execute a level 3 test command')

        # Build the default configuration model (Mandatory)
        self._mod_config = self.ModConfModel(param_exemple1='str',
                                             param_exemple2=1)
        ...
        if self.mod_config.param_exemple2 == 1:
            await self.ctx.Irc.Protocol.send_priv_msg(
                self.ctx.Config.SERVICE_NICKNAME,
                "Param activated",
                self.ctx.Config.SERVICE_CHANLOG)

    def unload(self) -> None:
        """### This method is called when you unload,
        or you reload the module (Mandatory)"""
        self.ctx.Commands.drop_command_by_module(self.module_name)
        return None

    async def asyncio_func(self) -> None:
        self.ctx.Logs.debug(
            f"Starting async method in a task: {self.__class__.__name__}")
        await asyncio.sleep(2)
        self.ctx.Logs.debug(f"End of the task: {self.__class__.__name__}")

    def cmd(self, data: list[str]) -> None:
        """All messages coming from the IRCD server will be handled using
    ...
    async def hcmds(self, user: str, channel: Optional[str],
                    cmd: list, fullcmd: Optional[list]) -> None:
        """All messages coming from the user commands (Mandatory)

        Args:
            user (str): The user who send the request.
            channel (Any): The channel from where is coming the message
                (could be None).
            cmd (list): The messages coming from the IRCD server.
            fullcmd (list, optional): The full messages coming from the
                IRCD server. Defaults to [].
        """
        u = self.ctx.User.get_user(user)
        c = (self.ctx.Channel.get_channel(channel)
             if self.ctx.Channel.is_valid_channel(channel) else None)
        if u is None:
            return None
            ...
            case 'test-command':
                try:
                    await self.ctx.Irc.Protocol.send_notice(
                        nick_from=dnickname,
                        nick_to=u.nickname,
                        msg="This is a notice to the sender ...")
                    await self.ctx.Irc.Protocol.send_priv_msg(
                        nick_from=dnickname,
                        msg="This is private message to the sender ...",
                        nick_to=u.nickname)
                    ...
                    await self.update_configuration('param_exemple1',
                                                    'my_value')

                    # Log if you want the result
                    self.ctx.Logs.debug("Test logs ready")
                    return None
