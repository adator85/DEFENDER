    ...
    if votec:
        ...
        if votec.vote_for >= votec.vote_against and votec.vote_for != 0:
            await uplink.ctx.Irc.Protocol.send_priv_msg(nick_from=dnickname, 
                                msg=f"User {uplink.ctx.Const.Colors.bold}{target_user}{uplink.ctx.Const.Colors.nogc} has {votec.vote_against} votes against and {votec.vote_for} votes for. For this reason, it\'ll be kicked from the channel",
                                channel=channel
                                )
            ...
            await uplink.ctx.Irc.Protocol.send_priv_msg(
                    nick_from=dnickname, 
                    msg=f"User {uplink.ctx.Const.Colors.bold}{target_user}{uplink.ctx.Const.Colors.nogc} has {votec.vote_against} votes against and {votec.vote_for} votes for. For this reason, it\'ll remain in the channel",
                    channel=channel
                    )
